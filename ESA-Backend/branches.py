    ...
    for sub in branch_list:
        ...
        for p in range(1, ws.max_row+1):
            if (ws.cell(row=p, column=4).value == sub):
                ...
                regno = nm[-11:-1:1]
                email = ws.cell(row=p, column=13).value  # assuming email is in column M (13th column)
                ...
                ws_branchMain.cell(row=r, column=5).value = email  # Adding email to column M
                r += 1
        codeno = regno[5:7]
            ...
            for p in range(1, wb_branchMain.max_row+1):
                if (wb_branchMain.cell(row=p, column=4).value == slot):
                    if (wb_branchMain.cell(row=p, column=2).value[3:5] == year_list[-1]):
                        ...
                        wb_branchRegular.cell(row=r, column=5).value = wb_branchMain.cell(
                            row=p, column=13).value  # Adding email to column M
                        r += 1
                    else:
                        ...
                        wb_branchSupply.cell(row=check_supply, column=3).value = wb_branchMain.cell(
                            row=p, column=5).value
                        wb_branchSupply.cell(row=check_supply, column=5).value = wb_branchMain.cell(
                            row=p, column=13).value  # Adding email to column M
                        check_supply += 1
