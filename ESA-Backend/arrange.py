...
for i in slot_list:
    ...
    for code in code_list:
        ...
        for r in range(1, check+1):
            ...
            ws_slotMain.cell(row=p, column=3).value = code  # Add subject code
            # Adding column E from the loaded workbook
            ws_slotMain.cell(row=p, column=5).value = ws_branch_reg.cell(
                row=r, column=5).value
            p += 1
        ...
        for r in range(check+1, ws_branch_reg.max_row+1):
            ...
            ws_slot_splyMain.cell(row=b, column=2).value = ws_branch_reg.cell(
                row=r, column=2).value
            ws_slot_splyMain.cell(row=b, column=3).value = code 
            # Adding column E from the loaded workbook
            ws_slot_splyMain.cell(row=b, column=5).value = ws_branch_reg.cell(
                row=r, column=5).value
            b += 1
        ...
        if (check_supply == 1):
            for r in range(1, ws_branch_sply.max_row+1):
                ...
                ws_slot_splyMain.cell(row=b, column=3).value = code 
                # Adding column E from the loaded workbook
                ws_slot_splyMain.cell(row=b, column=5).value = ws_branch_sply.cell(
                    row=r, column=5).value
                b += 1
        wb_slot.save('.\\updatedExcels\\'+date+'_'+time+'.xlsx')
    ...
    for j in sh_nm:
        ...
        room_sheet.cell(row=1,column=5,value='Sub. code')
        room_sheet.cell(row=1,column=4,value='Reg. No')
        room_sheet.cell(row=1,column=3,value='Email Id')
...
def send_seat_numbers(date,time):
        wb_branch = openpyxl.load_workbook('.\\updatedExcels\\'+date+'_'+time+'.xlsx')
        sender = EmailSender()
        ...
        for sheet_name in wb_branch.sheetnames[2:]:
            ...
            for row in data[1:]:
                ...
                sender.send_email(email, message)
                print(message)
                print("\n")
